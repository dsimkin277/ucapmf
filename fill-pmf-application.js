...
const PMF_URL = 'https://apply.myrmapp.com/single-step-apply/drubin';
...
async function sendReadyToSignEmail(applicantName) {
  ...
  await transporter.sendMail({
    ...
    html: `<p>The PMF application for <b>${applicantName}</b> has been filled out and the consent box checked.</p>
           <p>Please open the PMF form, draw the signature, and submit.</p>`,
  });
...
async function downloadAndExtractBankStatement(pdfUrl) {
  try {
    ...
    console.log('[PDF] Extracted text sample:', text.substring(0, 300));
...
async function fillPmfApplication(applicant, bankFilePaths) {
  ...
  console.log('[FILL] Contact info filled');
  ...
  console.log('[FILL] Business details filled');
  ...
  console.log('[FILL] Address filled');
  ...
  console.log(`[FILL] Done for ${applicant.firstName} ${applicant.lastName}. Consent checked.`);
  ...
  try {
    ...
    for (const sub of submissions) {
      ...
      for (const url of bankUrls) {
        console.log(`[PDF] Extracting from: ${url}`);
        bankStatementData = await downloadAndExtractBankStatement(url);
        if (bankStatementData?.addressLine1) {
          console.log('[PDF] Address found:', JSON.stringify(bankStatementData));
          break;
        }
      }
      ...
      for (let i = 0; i < bankUrls.length; i++) {
        const tmpPath = `/tmp/bank-statement-${sub.id}-${i}.pdf`;
        try {
      ...
      await fillPmfApplication(applicant, bankFilePaths);
